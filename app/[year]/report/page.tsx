      ...
      setTimeout(() => {
        ...
        if (selectedTool === 'claude-code') {
          text = encodeURIComponent(
            ...
            )}\n\nGet your wrapped at yearincode.xyz/${
              stats?.year
            }\n\n#YearInCode #ClaudeCode #CCUsage #DevTools #BuildInPublic #Wrapped2025`
          )
        } else {
          text = encodeURIComponent(
            `My ${stats?.year} GitHub Wrapped 🚀\n\n💪 ${formatNumber(
              stats?.totalTokens || 0
            )} contributions\n🔥 ${
              stats?.longestStreak || 0
            } day streak\n⚡ Top language: ${
              stats?.topModels[0]?.model || 'N/A'
            }\n\nGet your GitHub wrapped at yearincode.xyz/${
              stats?.year
            }\n\n#YearInCode #GitHub #GitHubWrapped #DevTools #BuildInPublic #Wrapped2025`
          )
        }
            ...
            <div className="flex flex-col md:flex-row gap-8 md:items-center">
              ...
              <div className="flex-1 flex flex-col justify-start">
                      ...
                      <div className="flex items-baseline gap-1 min-w-0">
                        ...
                        {githubUsername && (
                          <a
                            ...
                            className="text-sm text-gray-400 hover:text-gray-300 transition-colors truncate"
                          >
                            /{githubUsername}
                          </a>
                        )}
                ...
                <div className="mb-6 md:mb-8">
                      ...
                      <p className="text-base md:text-lg font-bold flex items-center gap-2">
                        ...
                        <span className="bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 text-transparent bg-clip-text">
                          ...
                          <span className="text-xs md:text-sm font-medium">
                            {getOrdinalSuffix(stats.yearsOfCoding)}
                          </span>{' '}
                          Year In Code
                        </span>
                      </p>
                  ...
                  {/* Mobile Horizontal Heatmap (only on mobile) */}
                        ...
                        month.map((day, dayIndex) => {
                          const getHeatmapStyle = () => {
                            ...
                            if (selectedTool === 'github') {
                              if (day.level === 1)
                                return { backgroundColor: '#0e4429' }
                              if (day.level === 2)
                                return { backgroundColor: '#006d32' }
                              if (day.level === 3)
                                return { backgroundColor: '#26a641' }
                          ...
                          const getHeatmapClass = () => {
                            if (selectedTool !== 'github') {
                              if (day.level === 0) return 'bg-gray-700/30'
                              if (day.level === 1) return 'bg-orange-400/40'
                              if (day.level === 2) return 'bg-orange-400/60'
                              if (day.level === 3) return 'bg-orange-500/80'
                          ...
                          return (
                            <div
                              ...
                              className={`w-2 h-2 rounded-full ${getHeatmapClass()}`}
                              style={getHeatmapStyle() || undefined}
                              title={day.date}
                            />
                          )
                        })
              ...
              {/* Right Column - Vertical Heatmap (12 Months - Desktop only) */}
                ...
                {heatmapMonths.map((month, monthIndex) => (
                  <div key={monthIndex} className="flex flex-col gap-[5px]">
                    {month.map((day, dayIndex) => {
                      const getHeatmapStyle = () => {
                        if (day.level === 0) {
                          return { backgroundColor: 'rgba(55, 65, 81, 0.3)' }
                        }

                        if (selectedTool === 'github') {
                          // Exact GitHub contribution colors
                          if (day.level === 1)
                            return { backgroundColor: '#0e4429' }
                          if (day.level === 2)
                            return { backgroundColor: '#006d32' }
                          ...
                          // Claude Code orange colors (keep as classes)
                          return null
                        }
                      }
                      ...
                      return (
                        <div
                          key={dayIndex}
                          className={`w-3.5 h-3.5 rounded-full ${getHeatmapClass()}`}
                          style={getHeatmapStyle() || undefined}
                          title={day.date}
                        />
                      )
