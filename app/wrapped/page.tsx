  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white flex items-center justify-center p-8">
      <div className="w-full max-w-4xl">
        {/* Main Card */}
        <div className="bg-gray-800/30 backdrop-blur-sm rounded-3xl p-12 border border-gray-700/50 shadow-2xl">
          {/* Header */}
          ...
          {/* Models Section */}
          <div className="mb-12">
            <h2 className="text-gray-400 text-lg mb-4">Models</h2>
            <div className="space-y-3">
              {stats.topModels.slice(0, 3).map((model, index) => (
                <div key={index} className="flex items-center gap-4">
                  <span className="text-4xl font-light text-gray-500 w-8">
                    {index + 1}
                  </span>
          ...
          {/* Stats Grid */}
          <div className="grid grid-cols-2 gap-x-32 gap-y-8 mb-12">
            {/* Left Column */}
            <div>
              <div className="text-gray-400 text-lg mb-2">Agents</div>
              <div className="text-5xl font-light">{formatNumber(stats.totalSessions)}</div>
            </div>

            {/* Right Column */}
            <div>
              <div className="text-gray-400 text-lg mb-2">Tabs</div>
              <div className="text-5xl font-light">{stats.activeDays}</div>
            </div>

            {/* Bottom Left */}
            <div>
              <div className="text-gray-400 text-lg mb-2">Tokens</div>
              <div className="text-5xl font-light">{formatNumber(stats.totalTokens)}</div>
            </div>

            {/* Bottom Right */}
            <div>
              <div className="text-gray-400 text-lg mb-2">Streak</div>
          ...
          <div className="flex items-center gap-2 text-gray-400 mb-8">
            ...
            <span className="text-lg">vibewrapped.com/{stats.year}</span>
          </div>
          ...
          <div className="flex gap-4">
            <button
              ...
              className="px-8 py-4 rounded-full bg-white text-black font-medium text-lg hover:bg-gray-100 transition-colors"
            >
              Download
            </button>
            <button
              onClick={copyToClipboard}
              className="px-8 py-4 rounded-full bg-gray-700/50 text-white font-medium text-lg hover:bg-gray-700 transition-colors"
            >
              Copy
            </button>
            <Link
              href="/upload"
              className="px-8 py-4 rounded-full bg-gradient-to-r from-orange-500 to-pink-600 text-white font-medium text-lg hover:from-orange-600 hover:to-pink-700 transition-colors"
            >
              Create New
            </Link>
          </div>
